...
public class GameService {
    ...
    public Game alterarGame(Game game) {
        gameRepository.recuperarGamePorIdComLock(game.getId())
            .orElseThrow(() -> new GameNaoEncontradoException(
                    "Game número " + game.getId() + " não encontrado."));
        return gameRepository.save(game);
    }
    ...
    public Game recuperarGamePorId(long id) {
        return gameRepository.recuperarGamePorId(id)
            .orElseThrow(() -> new GameNaoEncontradoException(
                "Game número " + id + " não encontrado."));
    }
